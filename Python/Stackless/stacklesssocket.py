...
#
# This code was written to serve as an example of Stackless Python usage. Feel free to email me
# with any questions, comments, or suggestions for improvement.
#
# This wraps the asyncore module and the dispatcher class it provides in order write a socket
# module replacement that uses channels to allow calls to it to block until a delayed event
# occurs.
#
# Not all aspects of the socket module are provided by this file. Examples of it in use can be
# seen at the bottom of this file.
#
# NOTE: Versions of the asyncore module from Python 2.4 or later include bug fixes and earlier
#       versions will not guarantee correct behaviour. Specifically, it monitors for errors on
#       sockets where the version in Python 2.3.3 does not.
#

# Possible improvements:
# - More correct error handling. When there is an error on a socket found by poll, there is no
#   idea what it actually is.
# - Launching each bit of incoming data in its own tasklet on the recvChannel send is a little
#   over the top. It should be possible to add it to the rest of the queued data
...
if "__all__" in stdsocket.__dict__:
    ...
    # WARNING: this function blocks and is not thread safe. The only solution is to spawn a
    # thread to handle all getaddrinfo requests. Implementing a stackless DNS lookup service is
    # only second best as getaddrinfo may use other methods.
...
# Someone needs to invoke asyncore.poll() regularly to keep the socket data moving. The
# "ManageSockets" function here is a simple example of such a function. It is started by
# StartManager(), which uses the global "managerRunning" to ensure that no more than one copy is
# running.
#
# If you think you can do this better, register an alternative to StartManager using
# stacklesssocket_manager(). Your function will be called every time a new socket is created;
# it's your responsibility to ensure it doesn't start multiple copies of itself unnecessarily.
#
...
class _fakesocket(asyncore.dispatcher):
    ...
    def __init__(self, realSocket):
        # This is worth doing. I was passing in an invalid socket which was an instance of
        # _fakesocket and it was causing tasklet death.
    ...
    def __del__(self):
        # There are no more users (sockets or files) of this fake socket, we are safe to close
        # it fully. If we don't, asyncore will choke on the weakref failures.
        self.close()

    # The asyncore version of this function depends on socket being set which is not the case
    # when this fake socket has been closed.
    ...
    def connect(self, address):
        ...
        if self.socket.type != SOCK_DGRAM and not self.connected:
            if not self.connectChannel:
                ...
                # Prefer the sender. Do not block when sending, given that there is a tasklet
                # known to be waiting, this will happen.
    ...
    def sendall(self, data, flags=0):
        # WARNING: this will busy wait until all data is sent It should be possible to do away
        # with the busy wait with the use of a channel.
        self.sendBuffer += data
    ...
    def recv(self, byteCount, flags=0):        
        # recv() must not concatenate two or more data fragments sent with send() on the remote
        # side. Single fragment sent with single send() call should be split into strings of
        # length less than or equal to 'byteCount', and returned by one or more recv() calls.
        ...
        if not self.connected:
            ...
            # Sockets which were connected, but no longer are, use up the remaining input.
            # Observed this with urllib.urlopen where it closes the socket and then allows the
            # caller to use a file to access the body of the web page.
    ...
    def recvfrom(self, byteCount, flags=0):
        ...
        # recvfrom() must not concatenate two or more packets. Each call should return the first
        # 'byteCount' part of the packet.
    ...
    def close(self):
        ...
        while self.recvChannel and self.recvChannel.balance < 0:
            # The closing of a socket is indicted by receiving nothing. The exception would have
            # been sent if the server was killed, rather than closed down gracefully.
    ...
    # Asyncore says its done but self.readBuffer may be non-empty so can't close yet. Do nothing
    # and let 'recv' trigger the close.
    ...
    def handle_read(self):
        try:
            if self.socket.type == SOCK_DGRAM:
                ...
                # Not sure this is correct, but it seems to give the right behaviour. Namely
                # removing the socket from asyncore.
                if not ret:
                    self.close()
            stackless.tasklet(self.recvChannel.send)(ret)
        except stdsocket.error, err:
            # If there's a read error assume the connection is broken and drop any pending
            # output
            if self.sendBuffer:
                self.sendBuffer = ""
...
if __name__ == '__main__':
    ...
    def TestTCPServer(address):
        ...
        i = 1
        while i < NUM_TESTS + 1:
            # No need to schedule this tasklet as the accept should yield most of the time on
    ...
    def TestMonkeyPatchUDP(address):
        ...
        try:
            def UDPServer(address):
                ...
                # Apparently each call to recvfrom maps to an incoming packet and if we only ask
                # for part of that packet, the rest is lost. We really need a proper unittest
                # suite which tests this module against the normal socket module.
